[CmdletBinding()]
param (
    [Parameter(HelpMessage = "Specify if this is just a check, if services and pools are to be restarted, leave this off")]
    [Switch]
    $Check
)

[string]$scriptDirectory = $PSScriptRoot
[string]$Global:InvokingScriptFileName = $MyInvocation.MyCommand.Name
...
foreach ($server in $ServerNames) {
    try {
        Write-Log -Message "Checking $Server"

        $PoolStatus = Invoke-Command -ComputerName $Server -ScriptBlock {
            if ($ENV:Computername -notlike "*INT*") {
                $Pools = Get-IISAppPool
            } else {
                $Pools = ""
            }
            $Services = Get-Service "Airwatch*"
        ...
        foreach ($Service in $PoolStatus.Services) {
            ...
            Write-Log -Message "$Server :: $($Service.Name) - $Status"

            if ($Start -eq $true) {
                Write-Log -Message "Restarting $($Service.Name)..."
                $restart = Invoke-Command -ComputerName $Server -ArgumentList $Service -ScriptBlock { param($Service)
        ...
        Write-Log -Message "Unable to check status of $Server" -Severity 3
    }
    if ($Server -notlike "*INT") {
        try {

            foreach ($Statuses in $PoolStatus.Pools) {
                ...
                Write-Log -Message "$Server :: $($Statuses.Name) - $Status"

                if ($Start -eq $true) {
                    Write-Log -Message "Restarting $($Statuses.Name)..."
                    $restart = Invoke-Command -ComputerName $Server -ArgumentList $Statuses -ScriptBlock { param($Statuses)
                        Start-WebAppPool -Name "$($Statuses.Name)"
                        Get-IISAppPool -Name "$($Statuses.Name)" }
